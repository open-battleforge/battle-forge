...
export default React.memo((props) => {
  ...
  const isSkirmish = gameType === "skirmish" || gameType === "racers";
                      ...
                      .map((optItem, optIdx) => {
                        ...
                        return {
                          ...
                          text:
                            optItem.text +
                            ` (${selections} ${
                              selections > 1 ? "selections" : "selection"
                            })`,
                        };
                      })
                      .filter((optItem, optIdx) => {
    ...
    forces.forEach((force) => {
      ...
      forceUnits.forEach((unit) => {
        unit?.rules?.forEach((rule) => {
          if (rule?.id === "limited" && limitedUnitMap[unit?.name]) {
            limitedUnitMap[unit?.name] = {
              ...unit,
              limitedCount: limitedUnitMap[unit?.name]?.limitedCount + 1,
            };
          } else if (rule?.id === "limited" && !limitedUnitMap[unit?.name]) {
            limitedUnitMap[unit?.name] = {
              ...unit,
              limitedCount: 1,
            };
      ...
      (Object.values(limitedUnitMap) || []).forEach((unit) => {
        const unitLimit = find(
          get(unit, "rules", []),
          (rule) => rule.id === "limited"
        )?.x;
      ...
      forceCategories.forEach((catKey) => {
        ...
        if (unitCatCount < categoryData.min) {
          ...
          validationErrors.push({
            type: "error",
            text: `${name} requires ${unitDiff} more ${category.name} unit${
              unitDiff > 1 ? "s" : ""
            }`,
          });
        }
    ...
    <>
      ...
      <Container>
        ...
        {!editMode && (
          ...
          >
            <CardHeader
              onClick={() => setCollapseTracker(!collapseTrackers)}
              sx={{
            ...
            <CardContent
              sx={{ px: 1 }}
              style={{ display: collapseTrackers ? "none" : "" }}
            >
              <Stack direction="column" spacing={2}>
                <FormGroup>
                  <InputNumber
                    ...
                    type="number"
                    color="primary"
                    value={Number(strategyPoints) || 0}
                    onChange={(value) => setStrategyPoints(value)}
                    allowReset
                  />
                </FormGroup>
                <FormGroup>
                  <InputNumber
                    ...
                    type="number"
                    color="primary"
                    value={Number(victoryPoints) || 0}
                    onChange={(value) => setVictoryPoints(value)}
                    allowReset
                  />
                </FormGroup>
              </Stack>
        ...
        {forces.map((force, index) => {
          ...
          return (
            ...
            >
                  ...
                  <>
                    <CardHeader
                      ...
                      title={
                        <ListItem
                          secondaryAction={
                            !!editMode && <IconButton sx={{ color: "inherit", mr: -1 }}>
                              <MoreVertIcon />
                            </IconButton>
                          }
                        >
              ...
              >
                <>
                  {(!!factionStrategies.length || !!factionPowers.length) && <ListItem key={index} disablePadding>
                    <ListSubheader
